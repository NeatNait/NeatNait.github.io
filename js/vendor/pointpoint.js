...
(function($){
	...
	$.fn.pointPoint = function(prop){
      ...
      "maxY" : 0
        ...
        //translate avoids the glitchs in the animation with margins
				props.transform  = 'translate('+(mouse.x+options.distance*Math.cos(deg)-8)
            +'px, '+(mouse.y-options.distance*Math.sin(deg))+'px)'
            +' rotate('+(-deg*rad_to_deg)+'deg)';
				props.opacity    = op;
